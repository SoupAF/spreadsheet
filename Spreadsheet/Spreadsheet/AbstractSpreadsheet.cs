...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        protected IEnumerable<String> GetCellsToRecalculate(ISet<String> names)
        {
            ...
            {
                //For every name in the list of cells tht hasn't been added to the list already, visit it
                if (!visited.Contains(name))
                {
                    Visit(name, name, visited, changed);
                }
            }
            //Return an in order list of all the cells that need to be changed
            return changed;
        }
        ...
        {
            //Add the current cell's name to the list of visited cells
            visited.Add(name);
            //For every dependent of the cell name
            foreach (String n in GetDirectDependents(name))
            {
                //If two cells depend on one another, throw an exception
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //Otherwise, add the visited cells name to the list of cells to recalcualte and visit that cell's dependants
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //Add the cell to the list of cells that need to be updated
